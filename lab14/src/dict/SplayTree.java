...
public class SplayTree implements Dictionary {
    ...
    private void zigZig(BinaryTreeNode node) {
        ...
        if (node == node.parent.leftChild) {
            rotateRight(node);
            rotateRight(node);
            rotateRight(node.rightChild.leftChild);
        } else {
            rotateLeft(node);
            rotateLeft(node);
            rotateLeft(node.leftChild.rightChild);
        }
    }
    ...
    private void splayNode(BinaryTreeNode node) {
        ...
        while (node.parent != null) {
            ...
            BinaryTreeNode grand=parent.parent;
            if(grand==null) zig(node);
            else if(((node==parent.leftChild)&&(parent==grand.rightChild))||((node==parent.rightChild)&&(parent==grand.leftChild)))
                zigZag(node);
            else zigZig(node);
        }
     ...
     *  a matching key is found (meaning th
     *  at key1.compareTo(key2) == 0), return
    ...
    public BinaryTreeNode min(BinaryTreeNode x){
        //find the minimum node within the trees rooted here
        if(x.leftChild==null) return x;
        else return min(x.leftChild);
    }
    public BinaryTreeNode deleteMin(BinaryTreeNode x){
    ...
    public BinaryTreeNode removeHelper(BinaryTreeNode x,Comparable key) {
        if(x==null)   return null;
        int cmp=key.compareTo(x.entry.key());
        if  (cmp < 0) x.leftChild =removeHelper(x.leftChild,key);
        else if (cmp > 0) x.rightChild = removeHelper(x.rightChild, key);
        else {
