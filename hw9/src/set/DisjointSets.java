...
public class DisjointSets {
     ...
     **/
    public void union(int root1, int root2) {//union by size
        if(array[root1]>=0||array[root2]>=0) {
            System.out.println("root1 or root2 is not root!!!");
            return;
        }
        if(root1==root2){
            System.out.println("you are unionning the same sets!!");
            return;
        }
    ...
    public int find(int x) {
        if(x>=array.length||x<0){
            System.out.println("there is no such item in Disjont sets");
            return -1;
        }
        if (array[x] < 0) {
