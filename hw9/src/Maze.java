...
public class Maze {
    ...
    public Maze(int horizontalSize, int verticalSize) {
       ...
       int k=0;
        ...
        for (int x = 1; x < numInSameSet; x *= 2) {
            for (int y = 0; x + y < numElements; y += 2 * x) {
                set1 = s.find(y);
                set2 = s.find(x + y);
                s.union(set1, set2);
            }
...
**/
        ...
        //Firstly create an array in which every wall (horizontal
        //and vertical) is represented.
        int myWalls[]=new int[horiz*(vert-1)+vert*(horiz-1)];
        int k=0;
        int l=-1*horiz*(vert-1);
        while(l<vert*(horiz-1)){
            myWalls[k]=l;
            k++;
            l++;
        }

        //Scramble the walls by reordering them into a random permutation.
         for(int w=myWalls.length;w>0;w--){
            int r=randInt(w);
             int Last = myWalls[w-1];
            int rWall=myWalls[r];
            myWalls[r]=Last;
            myWalls[w-1]=rWall;
         }
        // Visit the walls in the (random) order in which they appear in the array.
        for(i=0;i<myWalls.length;i++){
             int c=myWalls[i];
             if(c<0){//then this is hWalls(x=xTop,y=yTop)
                 c=-1*(c+1);
                 int xTop=c/(vert-1);
                 int yTop=c%(vert-1);
                 int xBottom=xTop;
                 int yBottom=yTop+1;
                 int cTop=xTop+yTop*horiz;
                 int cBottom=xBottom+yBottom*horiz;
                 if(myMaze.find(cTop)!=myMaze.find(cBottom)){
                     hWalls[xTop][yTop]=false;
                     myMaze.union(myMaze.find(cTop),myMaze.find(cBottom));
                 }
             }
             else{//then this is vWalls(x=xLeft,y=yLeft)
                 int xLeft=c%(horiz-1);
                 int yLeft=c/(horiz-1);
                 int xRight=xLeft+1;
                 int yRight=yLeft;
                 int cLeft=xLeft+yLeft*horiz;
                 int cRight=xRight+yRight*horiz;
                 if(myMaze.find(cLeft)!=myMaze.find(cRight)){
